...
public:
  ...
  public:
    InletVelocity()
      : Function<dim>(dim + 1) // 3 components for velocity and 1 for pressure to have compatibilità con FE
    {}
  ...
  public:
    ...
    {
      velocity_stiffness = &velocity_stiffness_; //Matrix A of the precond
      pressure_mass      = &pressure_mass_; //Matrix M of the precond

      //Since i have to solve two linear system as Ay=x_u and My= x_p i assigne two preconditioners
    ...
    //dst is where we are going to store the result of Precond^-1 * x
    //src is where we are applying the precond
    void
    vmult(TrilinosWrappers::MPI::BlockVector       &dst,
          const TrilinosWrappers::MPI::BlockVector &src) const
    {
      //Since i need to solve the two linear systems i need a Solver
      //Both A , M_p are symm so i use CG
        
      SolverControl                           solver_control_velocity(1000,
                                            1e-2 * src.block(0).l2_norm());
      SolverCG<TrilinosWrappers::MPI::Vector> solver_cg_velocity(
        solver_control_velocity); //Initialization

      //Solve precond: A^-1 * x_u
      solver_cg_velocity.solve(*velocity_stiffness,
                               dst.block(0),
                               src.block(0),
                               preconditioner_velocity);

      //Idem Solve Precond: M_p^-1 * x_p
  ...
  protected:
    ...
    TrilinosWrappers::PreconditionILU preconditioner_velocity; //to solve the linear system of the precond block associated to A

    // Pressure mass matrix.
    const TrilinosWrappers::SparseMatrix *pressure_mass;

    // Preconditioner used for the pressure block.
    TrilinosWrappers::PreconditionILU preconditioner_pressure; // to solve the lin sys of the precond block associated to M_p
  };
  ...
  class PreconditionBlockTriangular
  {
               ...
               const TrilinosWrappers::SparseMatrix &B_)
    {
      ...
      //Since i have to solve two linear system as Ay_u=x_u and 1/vMy= x_p - By_u i assigne two preconditioners
      preconditioner_velocity.initialize(velocity_stiffness_);
      preconditioner_pressure.initialize(pressure_mass_);

      tmp.reinit(pressure_mass_.locally_owned_range_indices(),pressure_mass_.get_mpi_communicator());
    }
          ...
          const TrilinosWrappers::MPI::BlockVector &src) const
              {
                  ...
                  SolverCG<TrilinosWrappers::MPI::Vector> solver_cg_velocity(
                  solver_control_velocity); //Initialization
                  //it's the same as before for the 1st precond
                  //Solve precond: A^-1 * x_u = yu
                  solver_cg_velocity.solve(*velocity_stiffness,
                               dst.block(0),
                               src.block(0),
                               preconditioner_velocity);
                  ...
                  //tmp = -1.0 * tmp + xp
                  tmp.sadd(-1.0,src.block(1)); //-1.0 * tmp + xp
                  

                  // Solve Precond: M_p * y_p = tmp
                   SolverControl                           solver_control_pressure(1000,
                                            1e-2 * src.block(1).l2_norm());
                  SolverCG<TrilinosWrappers::MPI::Vector> solver_cg_pressure(
                                            solver_control_pressure);
    ...
    // Temporary vector.
    mutable TrilinosWrappers::MPI::Vector tmp; //to store new residual
  };
