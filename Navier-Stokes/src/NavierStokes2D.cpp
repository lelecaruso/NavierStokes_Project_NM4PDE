...
void NavierStokes::assemble_Newton()
{
  pcout << "===============================================" << std::endl;
  pcout << "Assembling the Newton system" << std::endl;
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
  {
    ...
    for (unsigned int q = 0; q < n_q; ++q)
    {
      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
      {
        for (unsigned int j = 0; j < dofs_per_cell; ++j)
        {
          ...
          cell_matrix(i, j) +=
              (
                //termine con gradiente sol prec
                velocity_gradient_loc[q] *
                   fe_values[velocity].value(j, q) *
                   fe_values[velocity].value(i, q) 
                   +
                   //termine con gradiente delta
               fe_values[velocity].gradient(j, q) *
                   velocity_loc[q] *
                   fe_values[velocity].value(i, q) 
                   ) *
    ...
    if (cell->at_boundary())
    {
      for (unsigned int f = 0; f < cell->n_faces(); ++f)
      {
        if (cell->face(f)->at_boundary() && false) // NO NEUMAN
        {
  ...
  pressure_mass.compress(VectorOperation::add);
   ...
   pcout << "the Newton system is assembled" << std::endl;
...
void NavierStokes::solveLinearSystem()
{
  ...
  preconditioner.initialize(jacobian_matrix.block(0,0),pressure_mass.block(0,0));
...
void NavierStokes::solveNewtonMethod()
{
  pcout << "===============================================" << std::endl;
  // parametri
  const unsigned int n_max_iters = 100; //trial 
...
void NavierStokes::solveNewton()
{
  
  pcout << "===============================================" << std::endl;

  pcout << "newton for steady case  " << std::flush;
...
void NavierStokes::output_Newton()

{
  ...
  const std::string output_file_name = "output-Newton-Navierstokes";
  data_out.write_vtu_with_pvtu_record("./",
                                      output_file_name,
                                      0,
                                      MPI_COMM_WORLD);
