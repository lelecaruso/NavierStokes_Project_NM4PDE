...
// approximate Simple Correct
  ...
  public:
               ...
               )
    {
      ...
      diag_M.reinit(sol_owned.block(0));
      neg_diag_M_inv.reinit(sol_owned.block(0));

      for (unsigned int i : diag_M.locally_owned_elements())
      {
        double temp = M->diag_element(i); //no lump for better performance
        neg_diag_M_inv[i] = - 1.0 / temp;
      }

      //S_tilde = BD(^-1)B.T
      B->mmult(neg_S, *B_T, neg_diag_M_inv); //Note: we need -S_tilde, so we use - D(^-1)
          ...
          const TrilinosWrappers::MPI::BlockVector &src) const
    {
      ...
      const double tol = 1e-2;
       ...
       // Prepare Temp to hold partial results
        tmp.reinit(src); 

        // Step 1) 
        // This computes an approximate inverse of F applied to the first block of src.
        SolverControl solver_control_F(maxit, tol * src.block(0).l2_norm());
        SolverGMRES<TrilinosWrappers::MPI::Vector> solver_F(solver_control_F);
        solver_F.solve(*F, dst.block(0), src.block(0), preconditioner_F);

        // Step 2) 
        // Then update this temporary vector by incorporating the effect of -B.
        B->vmult(dst.block(1), dst.block(0));
        dst.block(1).sadd(-1.0, src.block(1)); // tmp.block(1) = -B * dst.block(0) + src.block(1)
        tmp.block(1) = dst.block(1);

        // Step 3)
        // Calculates the second block of src  by inverting negS_matrix.
        SolverControl solver_control_S(maxit, tol * tmp.block(1).l2_norm());
        SolverGMRES<TrilinosWrappers::MPI::Vector> solver_S(solver_control_S);
        solver_S.solve(neg_S, dst.block(1), tmp.block(1), preconditioner_S);

        // Step 4) 
        // Scale the primary component by the original F matrix entries.
        F->vmult(dst.block(0),dst.block(0));

        // Step 5)
        B_T->vmult(tmp.block(0), dst.block(1));
        //dst.block(0).sadd(-1,tmp.block(0));
        dst.block(0) -= tmp.block(0);


        tmp.block(0) = dst.block(0); //RHS
        
        // Step 6)
        // Finalize the update of the primary component by scaling it with Finv
        SolverControl solver_control_F1(maxit, tol * tmp.block(0).l2_norm());
        SolverGMRES<TrilinosWrappers::MPI::Vector> solver_F1(solver_control_F1);
        solver_F1.solve(*F, dst.block(0), tmp.block(0), preconditioner_F);   
    }
  ...
  protected:
    ...
    TrilinosWrappers::MPI::Vector neg_diag_M_inv;
    TrilinosWrappers::MPI::Vector diag_M;
